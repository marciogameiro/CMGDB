...
template <typename T>
template <bool t_sumup, bool t_inc, class t_iter>
inline uint64_t elias_gamma<T>::decode(uint64_t const * d, size_type const start_idx, size_type n, t_iter it)
{
    d += (start_idx >> 6);
    uint64_t value = 0;
    size_type i = 0;
...
template <typename T>
inline uint64_t
elias_gamma<T>::decode_prefix_sum(uint64_t const * d, size_type const start_idx, size_type const end_idx, size_type n)
{
    if (n == 0)
        return 0;
...
template <typename T>
inline uint64_t elias_gamma<T>::decode_prefix_sum(uint64_t const * d, size_type const start_idx, size_type n)
{
    if (n == 0)
        return 0;
    d += (start_idx >> 6);
